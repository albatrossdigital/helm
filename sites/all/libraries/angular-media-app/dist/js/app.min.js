...
!function() {
    angular.module("app", [ "app.core", "app.flickr", "ui.router", "ngResource", "infinite-scroll", "angularFileUpload", "ngJcrop" ]).run([ "$rootScope", "$state", "$stateParams", "$window", "$location", "$timeout", function($rootScope, $state, $stateParams) {
        $rootScope.settings = {}, $rootScope.files = {}, $rootScope.flickrApiKey = "5202f8e46861f39f55bedfa2374a41d8", 
        $rootScope.apiUrlUpload = "http://liftoff.local/api/angular-media/", $rootScope.apiUrlBrowse = "http://liftoff.local/api/angular-media/", 
        $rootScope.appUrl = "", $rootScope.multiple = !1, $rootScope.cardinality = 1, $rootScope.tabs = {
            upload: {
                key: "upload",
                title: "Upload"
            },
        ...
        }, $rootScope.defaultTabs = [ "upload", "site", "flickr" ], $rootScope.$state = $state, 
        $rootScope.$stateParams = $stateParams, $rootScope.addFiles = function(files) {
            ...
            jQuery("#" + $rootScope.activeField + "_media").trigger("change"), $state.go("base");
    ...
    uploader.onAfterAddingFile = function(fileItem) {
        ...
        $rootScope.addFiles($scope.selected), $scope.selected = [], $event.preventDefault();
    }, $scope.fetch = function($event) {
        var file = new CoreFile({
            external: "https://www.youtube.com/watch?v=HZEChv1AaOk"
        });
        console.log("FILE", $scope.externalFile), $scope.externalFile = "asdf", file.$save(function(data) {
            console.log("NEW FILE", data), $rootScope.addFiles([ data ]);
    ...
    $scope.init = function(params) {
        var fieldName = void 0 != params.fieldName ? params.fieldName : $rootScope.fieldName;
        $rootScope.activeField = fieldName, $rootScope.settings[fieldName] = {}, $rootScope.settings[fieldName].apiUrl = void 0 != params.apiUrl ? params.apiUrl : $rootScope.apiUrl, 
        $rootScope.settings[fieldName].cardinality = void 0 != params.cardinality ? parseInt(params.cardinality) : $rootScope.cardinality, 
        $rootScope.settings[fieldName].multiple = 1 != $rootScope.cardinality, $rootScope.settings[fieldName].allowedTypes = void 0 != params.allowedTypes ? params.allowedTypes : void 0, 
        $rootScope.settings[fieldName].cropRatio = void 0 != params.cropRatio ? params.cropRatio : void 0, 
        $rootScope.settings[fieldName].allowedSchemes = void 0 != params.allowedSchemes ? params.allowedSchemes : void 0, 
        $rootScope.settings[fieldName].addlFieldName = void 0 != params.addlFieldName ? params.addlFieldName : void 0, 
        $rootScope.files[fieldName] = void 0 != params.files ? params.files : $rootScope.files;
        var enabledTabs = params.tabs.length > 0 && "" != params.tabs[0] ? params.tabs : $rootScope.defaultTabs;
        console.log("et", enabledTabs);
        var tabs = [];
        angular.forEach(enabledTabs, function(tab) {
            void 0 != $rootScope.tabs[tab] && tabs.push($rootScope.tabs[tab]);
        }), $rootScope.settings[fieldName].tabs = tabs, $rootScope.settings[fieldName].tabs.slice().reverse(), 
    ...
    return {
        ...
        link: function($scope, $element, $attrs) {
            console.log("edit form", $scope.file), void 0 != $scope.file.filemime && ($scope.croppable = -1 != $scope.file.filemime.indexOf("image") && "none" != $rootScope.settings[$rootScope.activeField].cropRatio), 
            ...
            void 0 != $attrs.overwriteOnly && ($scope.overwriteOnly = !0), $scope.startEdit = function($event) {
                ...
                console.log($scope.file.attribution), $scope.file.$save(), $scope.editing = !1, 
                $event.preventDefault();
